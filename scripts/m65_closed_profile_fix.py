...
start = s.index('fn validate_mind_profile(profile: String) -> Result<String, std::io::Error> {')
end = s.index('\npub(crate) fn seed_id(world: &World)', start)
validation = '''fn validate_mind_profile(profile: String) -> Result<String, std::io::Error> {\n    if is_valid_mind_profile(&profile) {\n        Ok(profile)\n    } else {\n        Err(std::io::Error::other(\n            "mind profile must be one of: deterministic, pi, custom",\n        ))\n    }\n}\n\nfn is_valid_mind_profile(profile: &str) -> bool {\n    matches!(\n        profile,\n        DETERMINISTIC_MIND_PROFILE | "pi" | CUSTOM_MIND_PROFILE\n    )\n}\n'''

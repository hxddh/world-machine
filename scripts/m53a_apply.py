...
old_load = '        let manifest_path = manifest_path.as_ref().to_path_buf();\n        let json = fs::read_to_string(&manifest_path).map_err(|error| {'
new_load = '''        let requested_manifest_path = manifest_path.as_ref();
...
old_resolve = '''fn resolve_command(manifest_path: &Path, command: &str) -> Result<PathBuf, HostError> {
...
'''
new_resolve = '''fn resolve_command(manifest_path: &Path, command: &str) -> Result<PathBuf, HostError> {
    let command = PathBuf::from(command);
    let resolved = if command.is_absolute() {
        command
    } else {
...
old_archive = '        assert_eq!(session.archive().unwrap(), Some(archive));\n'
...
process_import = 'use std::process::{Child, ChildStdin, ChildStdout, Command, Stdio};\n'
if 'use std::sync::mpsc::' not in text:
    text = text.replace(
        process_import,
        process_import
...
max_const = 'pub const DEFAULT_MAX_RESPONSE_BYTES: usize = 16 * 1024 * 1024;\n'
if 'DEFAULT_REQUEST_TIMEOUT' not in text:
    text = text.replace(
        max_const,
        max_const + 'pub const DEFAULT_REQUEST_TIMEOUT: Duration = Duration::from_secs(5);\n',
    )

start = text.index('struct ProcessClient {')
end = text.index('fn read_bounded_line', start)
replacement = '''struct ProcessClient {
    child: Child,
...
impl ProcessClient {
    ...
    fn request(&mut self, request: PackRequest) -> Result<PackResponse, HostError> {
        ...
        let send_result = self
            .stdin
            .as_mut()
            .ok_or_else(|| io::Error::new(io::ErrorKind::BrokenPipe, "external Pack stdin is closed"))
            .and_then(|stdin| {
                stdin
...
if 'hung_process_is_timed_out_and_terminated' not in text:
    test = '''    #[cfg(unix)]
    #[test]
...
'''
    if marker not in text:
        raise SystemExit("descriptor-test marker not found")
...
if 'PACK_PROCESS="$ROOT/crates/world-pack-process"' not in b:
    b = b.replace(
        'PACK_PROTOCOL="$ROOT/crates/world-pack-protocol"\n',
        'PACK_PROTOCOL="$ROOT/crates/world-pack-protocol"\nPACK_PROCESS="$ROOT/crates/world-pack-process"\n',
    )
    b = b.replace(
        'pack_protocol_forbidden=("TinySociety"',
...
'''
    b = b.replace('if [[ -d "$PROJECTION" ]]; then\n', guard + 'if [[ -d "$PROJECTION" ]]; then\n', 1)

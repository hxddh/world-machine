...
pub fn compare_evidence_neighborhoods(
    left: &ProjectionSnapshot,
    right: &ProjectionSnapshot,
    root: SelectionId,
    max_depth: usize,
    ...
    let nodes = selections
        .into_iter()
        .filter_map(
            |selection| match (left_nodes.get(&selection), right_nodes.get(&selection)) {
...
mod tests {
    ...
    fn evidence_neighborhood_snapshot(
        relation_from: u64,
        relation_to: u64,
        event_entity: u64,
    ) -> ProjectionSnapshot {
    ...
    fn endpoint_role_changes_are_typed_edge_differences_even_when_nodes_match() {
        ...
        assert!(comparison
            ...
            .left_only
            .iter()
            .all(|edge| matches!(edge, StateEvidenceEdge::EntityRelation(_))));
    }
    ...
    fn root_visibility_is_preserved_in_local_comparison() {
        ...
        assert_eq!(
            compare_evidence_neighborhoods(&empty, &empty, root, 1),
            None
        );
    }
    ...
    fn removed_relation_endpoint_name_drift_does_not_change_tombstone_identity() {
        ...
        let right = snapshot(
            ...
            [relation_inspector_with_endpoints(
                ...
                "Removed",
                "2",
                (1, "Renamed Alice · Entity #1"),
                (2, "Renamed Bob · Entity #2"),
            )],
            vec![],
            vec![],
        );

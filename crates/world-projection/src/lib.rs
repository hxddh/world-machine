...
fn recorded_entity_change_rows(entity: EntityId, world: &World) -> Vec<InspectorRow> {
    world
        ...
        .map(|event| InspectorRow {
            label: SelectionId::Event(event.id).stable_key(),
            value: format!("World time {} · {}", event.world_time, humanize(&event.kind)),
        })
        .collect()
}
...
mod tests {
    ...
    fn entity_history_uses_stable_event_keys_and_typed_timeline_lookup() {
        ...
        assert_eq!(recorded.rows[0].label, "event-3");
        assert_eq!(recorded.rows[0].value, "World time 3 · Workspace Renamed");
        assert_eq!(recorded.rows[1].label, "event-1");

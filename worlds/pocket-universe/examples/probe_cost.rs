...
//!
//! ```bash
//! cargo run --release -p pocket-universe --example probe_cost
//! ```
//!
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    registry.register(pocket_universe::pocket_universe_registration())?;
    let mut world = registry.create(pocket_universe::POCKET_UNIVERSE_PACK_ID)?;
    world.handle(world_projection::ProjectionIntent::InvokeCommand(
        pocket_universe::SEED_MARS_COLONY_COMMAND.into(),
    ))?;

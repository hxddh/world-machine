...
- **A World window reads like a page, not a log.** What happened is told first, in order, as a short story; the choice in front of you is the one highlighted panel, and each option answers the pointer; how things stand follows. People, places, and a history grouped by moment sit in a sidebar that folds under the page in a narrow window. The World's name appears once, in one title bar with Branch and What if….
...
- **The World map shows.** It had collapsed to a one-pixel line in every release.

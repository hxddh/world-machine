...
- **World Machine has a Settings window.** ⌘, opens it, and it is where a World's voice lives now — on or off, whether it uses a program on your Mac or an API key, and the key itself. It says in plain words what each choice means, including that the key is the only setting there that sends anything off your Mac. ⌘, used to open About, which is not what that key does anywhere else on macOS; About is still in the menu.
- An API key is kept in your **login keychain**, not in any file World Machine writes, so a backup of your Worlds folder never carries one. You can see, inspect, or delete it yourself in Keychain Access under "World Machine · World voice".
- **A World can be given a voice with an API key instead of a local program**, which is the path for somebody who has never opened a terminal. This is the only feature on which a World's contents leave your Mac: one request per return, carrying the facts that return is about to show and nothing else — not your other Worlds, not your library, not your file names. Off unless you switch it on, and the privacy page now says exactly what goes and what does not.
- **Giving your Worlds a voice is now something you can switch on.** The Analyst settings gain a **World voice** row: once you have chosen a Pi program, turning it on means the Worlds you open from then on say what happened in their own words. Off by default, and while it is off nothing is sent anywhere.
- **A World can now be given a voice.** Point Pocket Universe at a local model and what it tells you happened is written about your World rather than picked from a table; leave it unset and everything reads exactly as it does today. What decides and what narrates are separate settings, because the mind runs twice a period and only ever picks one of two actions, while the voice runs once per return and writes what you read — so the combination worth having keeps the simulation instant and gives only the prose to the model.
- A World that has been given a voice now uses it for **everything a return shows you**, not just the latest thing that happened. Measured over twenty week-long returns, the lines describing trouble were stuck at exactly three phrasings each — there are three troubles per seed and one written line per stage — and now repeat not at all. The whole return still costs one request.

...
- **The Anchor Pub is a business.** It had a proprietor, a shop assistant the World had never recorded employing, and no customers at all — its only money was the reserve it was seeded with, draining from the first morning with no way to refill, which is why it was the first thing in town to run out. It now takes money over the counter and pays Sofia for the job she always had the title for. The custom is deliberately a little short of the wages: Anchor Pub running dry on its old schedule is what starts the chain the whole town turns on, and a solvent pub quietly deleted it.
- **People are where their work is, and they move when it changes.** Every resident's whereabouts used to be written once, when the World was seeded, and never again: Mara stood in front of her own shut bakery for the rest of the World. Now a resident stands at the place they work at, and on the quay when the place itself has shut — so the end of Harbour Town is the whole town standing on the quay together, and the picture changes because somebody walked as well as because something happened to them.
- **A World says how it is doing, in one number of its own choosing.** Tiny Society reports the money changing hands in the town over the last ten days. It is high while the catches and the wages and the bread money go round, it falls as each part of that circle breaks, and it is zero in a town that has come to rest. Packs that offer no figure are unaffected, and so is every saved World.
- **A town that has settled says so.** Harbour Town's only export is Jonas's catch; once the boat is gone the money stops coming in, the reserves drain, and a day arrives when nothing can happen. That ending is the World working as intended — but it used to arrive in silence. The World recorded its last event and went on advancing time for as long as anyone asked, producing nothing, while the return kept showing the same stale card. Coming to rest is now a thing that happens to the town, recorded like anything else, and the return says it. It is not an ending: reopening the bakery wakes the town, and when that money runs out it comes to rest again.
- **A workplace that cannot pay its people does something about it.** The Anchor Pub and the Island School each announced that they had run out of wages and then changed nothing at all: Leo stayed the `pub_owner` of a pub with an empty till, Emma stayed a teacher at a school that could not pay her, the daily scheduler skipped them in silence, and no further event about either was ever recorded. A workplace that runs out now shuts, and the job it could not pay for ends.
- **Any place can say it has shut, not only the bakery.** A drawing of the town showed two brightly lit shopfronts with nobody left working in them; the pub and the school now read as closed.
- **A World is drawn as the place it is.** The window showed a World as rows of text and a scatter of labelled boxes. It now draws the town: the shops along the quay, the jetty out over the water, and everybody standing where the World says they are. Whether the bakery is shut is something you see — shutters down, windows dark — rather than a line reading `Operating Status: closed`, and an empty mooring is an empty mooring.
- **The people in the news are the people in the picture.** Each thing that happened carries who it was about, so coming back lights up exactly those residents and leaves the rest of the town going about its day. Nothing searches a sentence for names.
- **A canvas can say where a thing is and how it is doing.** World Packs could only place things at loose coordinates and describe their condition in prose, which is why every drawing of a World came out as a list. They can now say what a thing is at and whether it is working, stopped, hurt or gone. Packs that say neither keep the plain boxes they had, and no saved World changes.
- The desktop drawing code is now checked on every build rather than only the Mac ones. It turns out to compile anywhere; excluding it meant a mistake in it waited for a macOS runner to find.

use micro_company::{
    micro_company_descriptor, micro_company_registration,
    micro_company_registration_with_agent_runtime_profile,
};
use std::env;
use std::error::Error;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let descriptor = micro_company_descriptor();
    ...
    if args.len() == 2 && args[0] == "--write-bundle" {
        write_current_exe_bundle(&descriptor, PathBuf::from(&args[1]))?;
        return Ok(());
    }
    if !args.is_empty() {
    ...
    match env::var(MIND_ENV).as_deref().unwrap_or("deterministic") {
        "deterministic" => serve_stdio(micro_company_registration())?,
        "pi" => {

...
3. **A local model writes it, across enough of the page to matter.** Route the program the Analyst panel already persists into the Pack process and replace the environment variable with a visible choice.
   Measured while building item 2, with a narrator standing in for a model: narrating only the latest consequence moves the repeat share from 81% to **73%**, because a briefing shows roughly seven lines and that is one of them. So this item is not "plug in a model" — it has to widen where a narrated line is preferred (the guidance and compass lines, and the return digest's other entries) until the acceptance number actually moves. The one-call-per-return budget is per return, not per line: one request can carry the whole page.

...
3. **A local model writes the news.** Route the program the Analyst panel already persists into the Pack process and replace the environment variable with a visible choice.
   Two measurements from building item 2 shape this. First, the 81% figure counts the whole page and over-states the problem: 421 of 586 lines are a status panel reporting facts that have not changed, and rewording those every visit would make a stable fact look like it moved. The honest target is the news lines — 65% repeat with the table, 56% with one narrated line per return. Second, the one-call-per-return budget is per *return*, not per line, so one request can carry every news line on the page rather than only the latest consequence.

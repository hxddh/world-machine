...
M228 closes one known false-ready state in that startup gate. Pi reports `get_state.model` as the selected Model object or `null`, and a `null` model makes Pi's prompt path fail before a model request can run. The World Analyst probe therefore rejects `model: null` before `ready` with a provider-neutral command error and tells the user to configure model/authentication in Pi. A missing or malformed `model` field is treated as protocol incompatibility and fails the contaminated process closed. A non-null model is not treated as proof that provider credentials, network access, quota, or the remote model service are healthy; checking those would require a real model request and remains outside startup readiness. Model identity, provider metadata, and credentials still never cross the M220 product boundary, and World Machine does not store API keys or duplicate Pi's model settings.
